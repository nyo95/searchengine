window.Quick = (function(){
  ...
  function tpl(p){
    ...
    const hasSel = !!(p.type && String(p.type).trim()!=='');
    const unknown = hasSel && !Object.prototype.hasOwnProperty.call(kinds, p.type);
    // Keep Unassigned selectable so the browser doesn't auto-pick first enabled option
    const un = hasSel ? '<option value="">(Unassigned)</option>' : '<option value="" selected>(Unassigned)</option>';
    let arr = Object.keys(kinds).sort().map(function(k){ return '<option value="'+k+'"'+(p.type===k?' selected':'')+'>'+k+' - '+(kinds[k]||'')+'</option>'; });
    if(unknown){ arr = ['<option value="'+p.type+'" selected>'+p.type+' - (Unknown)</option>'].concat(arr); }
    const opts = [un].concat(arr).join('');
    const sw = p.swatch||{};
    let swStyle = '';
