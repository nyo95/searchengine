window.Scheduler = (function(){
  ...
  function header(){
    return `<div class=\"toolbar flex items-center gap-2 mb-2\">`+
      ...
      `<button id="btn_generate" class="btn" title="Generate 2D Boards">Generate Boards</button>`+
      `</div>`+
  ...
  function typeOptionsLabel(sel){
    const kinds=State.kinds||{};
    const hasSel = !!(sel && String(sel).trim()!=='');
    const unknown = hasSel && !Object.prototype.hasOwnProperty.call(kinds, sel);
    // Keep Unassigned selectable to prevent browser choosing first enabled option
    const un = hasSel ? '<option value="">(Unassigned)</option>' : '<option value="" selected>(Unassigned)</option>';
    let opts = Object.keys(kinds).sort().map(k=>`<option value="${k}" ${sel===k?'selected':''}>${kinds[k]||k}</option>`);
    if(unknown){ opts = [`<option value="${sel}" selected>${sel} (Unknown)</option>`].concat(opts); }
    return [un].concat(opts).join('');
  }
  function currentColumns(){
    // Export columns mirroring Scheduler table layout.
    // Include a 'thumb' column (left empty in CSV by backend).
    return ['code','thumb','kind_label','brand','subtype','notes','sample_received'];
  }
  // Safeguard: some builds referenced column resizing helpers that may not be present.
  // Define no-op fallbacks to avoid breaking Scheduler render.
  function applySavedWidths(){ try { /* noop fallback */ } catch(e){} }
  function wireResizers(){ try { /* noop fallback */ } catch(e){} }
  function render(){
    const el = $('#tab-scheduler');
    try { el.innerHTML = header(); } catch (e) { console.error('header render failed', e); return; }
    try { const q = document.getElementById('sch_q'); if (q) q.addEventListener('input', ()=>{ if(searchDebounceTimer) clearTimeout(searchDebounceTimer); searchDebounceTimer=setTimeout(()=>{ searchDebounceTimer=null; scheduleRowsRender(); },150); }); } catch (e) {}
    try { const f = document.getElementById('sch_filter'); if (f) f.addEventListener('change', ()=>{ filterType = f.value || ''; scheduleRowsRender(); }); } catch (e) {}
    try { const g = document.getElementById('btn_generate'); if (g) g.addEventListener('click', ()=>{ __rpc('generate_boards', { size: 500, gap: 100 }); }); } catch (e) {}
    ...
    try { setTimeout(()=>{ try { scheduleRowsRender(); wireResizers(); } catch (_e) {} }, 0); } catch (e) {}
  }
  ...
  let renderRowsJob = null;
  let renderRowsScheduled = false;
  function scheduleRowsRender(){
    if(renderRowsScheduled){ return; }
    renderRowsScheduled = true;
    requestAnimationFrame(()=>{ renderRowsScheduled=false; renderRows(); });
  }
  function clearPendingBatch(){ if(renderRowsJob){ cancelAnimationFrame(renderRowsJob); renderRowsJob=null; } }
  function renderRows(){
    ...
    // Front-end guard: exclude unassigned and hidden regardless of backend flags
    let rows=(State.rows||[]).filter(function(r){ return r && r.type && String(r.type).trim()!=='' && !r.hidden; });
    ...
    try{
      if(!rows.length){
        tb.innerHTML = '<tr><td colspan="9" class="muted">No materials to show</td></tr>';
        wireRows();
        return;
      }
      tb.innerHTML='';
      let idx = 0;
      const chunk = ()=>{
        const end = Math.min(idx + RENDER_BATCH, rows.length);
        const temp=document.createElement('tbody');
        const bucket=[];
        for(; idx<end; idx++){ bucket.push(row(rows[idx])); }
        temp.innerHTML = bucket.join('');
        const newRows=Array.from(temp.children);
        const frag=document.createDocumentFragment();
        newRows.forEach(tr=>frag.appendChild(tr));
        tb.appendChild(frag);
        wireRows(newRows);
        if(idx < rows.length){
          renderRowsJob = requestAnimationFrame(chunk);
        }else{
          renderRowsJob = null;
        }
      };
  ...
  function wireRows(nodeList){
    const rows = nodeList ? Array.from(nodeList) : $$('#sch_rows tr');
    rows.forEach(tr=>{
    ...
    tr.querySelectorAll('.flag').forEach(btn=>{
      ...
      btn.addEventListener('click',()=>{ const locked = (tr.getAttribute('data-locked')==='1'); if(locked && ['sample','locked','received'].indexOf(key)===-1){ __toast('Locked: only sample/received/unlock'); return; } const on=!btn.classList.contains('on'); btn.classList.toggle('on'); if(window.__setFlag){ window.__setFlag(id,key,on); } else { const flags={}; flags[key]=on; __rpc('set_flags',{ ids:[id], flags:flags }); } });
    });
    const tsel=tr.querySelector('.t_type'); if(tsel){ tsel.addEventListener('change',()=>{ const val=tsel.value||''; if(val===''){ return; } markDirty(id,{ prefix:val }); }); }
    ...
    const trv=tr.querySelector('.t_revert'); if(trv){ trv.addEventListener('click',()=>{ delete (State.pending||{})[id]; applySavedWidths(); scheduleRowsRender(); wireResizers(); }); }
  }); }
